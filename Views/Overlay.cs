    ...
    public sealed partial class Overlay : Form {
        ...
        static Overlay() {
            ...
            for (int i = 0; i < fontNames.Length; i++) {
                ...
                using (Stream fontStream = typeof(Overlay).Assembly.GetManifestResourceStream($"FinalBeansStats.Resources.font.{fontNames[i]}")) {
            ...
            if (!Directory.Exists($"{Stats.CURRENTDIR}Overlay")) {
                ...
                using (Stream overlayStream = typeof(Stats).Assembly.GetManifestResourceStream("FinalBeansStats.Resources.overlay.background.png")) {
                ...
                using (Stream overlayStream = typeof(Stats).Assembly.GetManifestResourceStream("FinalBeansStats.Resources.overlay.tab_unselected.png")) {
                    byte[] overlaydata = new byte[overlayStream.Length];
                    overlayStream.Read(overlaydata, 0, (int)overlayStream.Length);
        ...
        public void UpdateDisplay(bool init = false) {
            this.ArrangeDisplay(init, string.IsNullOrEmpty(this.StatsForm.CurrentSettings.OverlayFixedPosition) ? this.StatsForm.CurrentSettings.FlippedDisplay : this.StatsForm.CurrentSettings.FixedFlippedDisplay, this.StatsForm.CurrentSettings.ShowOverlayTabs,
                ...
                string.IsNullOrEmpty(this.StatsForm.CurrentSettings.OverlayFixedPosition) ? this.StatsForm.CurrentSettings.OverlayWidth : this.StatsForm.CurrentSettings.OverlayFixedWidth,
                string.IsNullOrEmpty(this.StatsForm.CurrentSettings.OverlayFixedPosition) ? this.StatsForm.CurrentSettings.OverlayHeight : this.StatsForm.CurrentSettings.OverlayFixedHeight,
        ...
        public int GetDisplaySetting() {
            return (this.StatsForm.CurrentSettings.HideWinsInfo ? 4 : 0) + (this.StatsForm.CurrentSettings.HideRoundInfo ? 2 : 0) + (this.StatsForm.CurrentSettings.HideTimeInfo ? 1 : 0);
        }
            ...
            if (e.Button == MouseButtons.Left) {
                ...
                if (sender.Equals(this.picPositionNE)) {
                    if (this.isFixedPositionNe) {
                        ...
                        this.saveDisplayChange = false;
                        this.Location = new Point(screenLocation.X, 0);
                        this.saveDisplayChange = true;
        ...
        private void SetLocationPositionMenu(bool visibleTab, int lockOffset, bool flipped) {
            ...
            this.picPositionLock.Location = new Point(flipped ? (this.Width - this.picPositionLock.Width - 14) : 14, this.Height - (this.picPositionLock.Size.Height + 2) - lockOffset);
        }
        ...
        private void SetVisiblePositionLockButton(bool visible) {
            if (visible) { this.picPositionLock.Show(); } else { this.picPositionLock.Hide(); }
        }
        ...
        private void Overlay_Resize(object sender, EventArgs e) {
            if (this.saveDisplayChange) {
                this.StatsForm.CurrentSettings.OverlayWidth = this.Width;
                this.StatsForm.CurrentSettings.OverlayHeight = this.Height;
                this.StatsForm.CurrentSettings.OverlayFixedWidth = this.Width;
                this.StatsForm.CurrentSettings.OverlayFixedHeight = this.Height;
            }

            int lockButtonOffset = -6;
            if (this.StatsForm.CurrentSettings.LockButtonLocation == 0) {
                if (this.drawHeight > 99 || this.Height < 62) {
                    lockButtonOffset = (this.Height / 2) - 8;
                } else {
                    lockButtonOffset = this.Height - 40;
                }
            }
            this.SetLocationPositionMenu(this.drawHeight > 99, lockButtonOffset, this.StatsForm.CurrentSettings.FlippedDisplay);
        }
        ...
        private void SetRoundLabel(Image roundIcon, LevelType type, string roundName, int setting) {
            if (!Stats.InShow && Stats.IsQueuing && (setting == 1 || setting == 5)) {
                ...
                int roundNum = this.lastRound.Round > 0 ? this.lastRound.Round : 0;
                    ...
                    case 0:
                        if (this.StatsForm.CurrentSettings.PlayerByConsoleType) {
                            ...
                            if (this.StatsForm.CurrentSettings.CountPlayersDuringTheLevel) {
                                ...
                                } else if (type == LevelType.Race || type == LevelType.Hunt || type == LevelType.Team) {
                                    if (Stats.NumPlayersSucceeded > 0) {
                                        psCount = Stats.NumPlayersPsSucceeded;
                                        xbCount = Stats.NumPlayersXbSucceeded;
                                        swCount = Stats.NumPlayersSwSucceeded;
        ...
        private void SetFinishLabel(StatSummary summary, LevelType type, BestRecordType record, DateTime currentUtc, int setting) {
        ...
        private void UpdateInfo() {
            ...
            lock (this.StatsForm.CurrentRound) {
                ...
                if (this.lastRound != null && !string.IsNullOrEmpty(this.lastRound.Name)) {
                    bool isRoundInfoNeedRefresh = true; // Not optimized but needed for FinalBeans
                    ...
                    if (isRoundInfoNeedRefresh) {
                        ...
                        this.levelId = this.lastRound.Name;
                        ...
                        this.levelSummary = this.StatsForm.GetLevelInfo(this.levelId, this.levelType, this.recordType);
                    }

                    int overlaySetting = this.GetDisplaySetting();
                    ...
                    lock (this.StatsForm.LevelTimeLimitCache) {
                        this.levelTimeLimit = this.StatsForm.LevelTimeLimitCache.Find(l => string.Equals(l.LevelId, this.lastRound.RoundId))?.Duration ?? 0;
                    }

                    this.SetDurationLabel(this.levelTimeLimit, currentUtc, overlaySetting);
                    this.SetFinishLabel(this.levelSummary, this.levelType, this.recordType, currentUtc, overlaySetting);
                }
                this.Invalidate();
            }
        }
        ...
        public void ResetOverlaySize() {
            this.StatsForm.CurrentSettings.OverlayWidth = this.DefaultSize.Width;
            this.StatsForm.CurrentSettings.OverlayHeight = this.DefaultSize.Height;
            this.StatsForm.SaveUserSettings();
            this.UpdateDisplay();
        }

        public void ResetOverlayLocation() {
        ...
        public void ArrangeDisplay(bool initDisplay, bool flipDisplay, bool showTabs, bool hideWins, bool hideRound, bool hideTime, int colorOption, int lockButtonLocation, int? width, int? height, string serializedFont, string serializedFontColor) {
            ...
            int lockButtonOffset = -6;
            if (lockButtonLocation == 0) {
                if (showTabs || this.Height < 62) {
                    lockButtonOffset = (this.Height / 2) - 8;
                } else {
                    lockButtonOffset = this.Height - 40;
                }
            ...
            this.picPositionLock.Image = this.isPositionLock ? this.positionLockBlur : this.positionUnlockBlur;
            this.SetBlurPositionMenu();
            this.SetLocationPositionMenu(showTabs, lockButtonOffset, flipDisplay);
        ...
        public void FlipDisplay(bool flipped) {
            ...
            this.picPositionLock.Location = new Point(flipped ? (this.Width - this.picPositionLock.Width - 14) : 14, this.picPositionLock.Location.Y);
        }
            ...
            lock (DefaultFont) {
                ...
                using (Graphics g = Graphics.FromImage(newImage)) {
                    ...
                    if (string.IsNullOrEmpty(this.BackgroundResourceName)) {
                        background = Properties.Resources.background_frightful_final_ween;
                    } else {
                        if (overlayCustomized) {
                            ...
                            background = File.Exists(Path.Combine($"{Stats.CURRENTDIR}Overlay", $"{this.BackgroundResourceName}.png")) ? this.customizedBackground : Properties.Resources.background_frightful_final_ween;
                        } else {
                            background = (Bitmap)Properties.Resources.ResourceManager.GetObject(this.BackgroundResourceName) ?? Properties.Resources.background_frightful_final_ween;
                        }
                    }
                    ...
                    if (tabsDisplayed) {
                        ...
                        if (string.IsNullOrEmpty(this.TabResourceName)) {
                            tab = Properties.Resources.tab_unselected_frightful_final_ween;
                        } else {
                            if (overlayCustomized) {
                                ...
                                tab = File.Exists(Path.Combine($"{Stats.CURRENTDIR}Overlay", $"{this.TabResourceName}.png")) ? this.customizedTab : Properties.Resources.tab_unselected_frightful_final_ween;
                            } else {
                                tab = (Bitmap)Properties.Resources.ResourceManager.GetObject(this.TabResourceName) ?? Properties.Resources.tab_unselected_frightful_final_ween;
                            }
                        }

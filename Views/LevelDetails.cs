    ...
    public partial class LevelDetails : MetroFramework.Forms.MetroForm {
        ...
        private void LevelDetails_Load(object sender, EventArgs e) {
            ...
            if (string.Equals(this.LevelName, "Shows")) {
                ...
                this.BackImage = Properties.Resources.finalbeans_icon;
        ...
        public void LevelDetails_OnUpdatedLevelRows() {
            switch (this.statType) {
                ...
                case StatType.Levels when string.Equals(this.gridDetails.Name, "gridLevelsStats"):
                    LevelStats levelStats = this.StatsForm.StatDetails.Find(l => string.Equals(l.Id, this.LevelId));
        ...
        private void gridDetails_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e) {
            ...
            if (((Grid)sender).Columns[e.ColumnIndex].Name == "End") {
                ...
                if ((this.statType == StatType.Levels || this.statType == StatType.Rounds) && this.StatsForm.StatLookup.TryGetValue(info.Name, out LevelStats level)) {
        ...
        private void gridDetails_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e) {
            ...
            this.currentRoundDetails.Sort(delegate (RoundInfo one, RoundInfo two) {
                ...
                switch (columnName) {
                    ...
                    case "RoundIcon":
                    case "Name":
                        string nameCompareOne = Multilingual.GetLevelName(one.Name) ?? one.Name;
                        string nameCompareTwo = Multilingual.GetLevelName(two.Name) ?? two.Name;
                        int nameCompareCompare = nameCompareOne.CompareTo(nameCompareTwo);
                        return nameCompareCompare != 0 ? nameCompareCompare : showCompare == 0 ? roundCompare : showCompare;
                    case "Players":
        ...
        private void gridDetails_CellMouseEnter(object sender, DataGridViewCellEventArgs e) {
            ...
            if (this.statType == StatType.Shows && ((Grid)sender).Columns[e.ColumnIndex].Name == "ShowNameId") {
                ...
                for (int i = 0; i < levelNames.Length; i++) {
                    ...
                    strBuilder.Append($"• {Multilingual.GetWord("overlay_round_prefix")}{i + 1}{Multilingual.GetWord("overlay_round_suffix")} : {type}{levelStats.Name ?? levelNames[i].Substring(0, levelNames[i].Length - 3)}");

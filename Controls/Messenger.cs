...
using static FinalBeansStats.KeyValue;
using Timer = System.Windows.Forms.Timer;

namespace FinalBeansStats {
    /* Author: BlueMystic (bluemystic.play@gmail.com)  2024 */
    /* Adapted for FinalBeansStats by Micdu70/LicorneRoseTM  2025 */
    public static class Messenger {
        ...
        public static DialogResult MessageBox(
            ...
            MessageBoxDefaultButton _defaultButton = MessageBoxDefaultButton.Button1, Form owner = null, bool screenCentered = true) {
            SystemSounds.Beep.Play();

            Form form = new Form {
                Icon = Properties.Resources.fbstats,
                FormBorderStyle = FormBorderStyle.FixedDialog,
                StartPosition = screenCentered ? FormStartPosition.CenterScreen : FormStartPosition.CenterParent,
                MaximizeBox = false,
                MinimizeBox = false,
                Text = title,
                Width = 340,
            ...
            systemFont = Overlay.GetMainFont(14);
            ...
            using (Graphics g = form.CreateGraphics()) {
                ...
                for (int c = CmdButtons.Count - 1; c >= 0; c--) {
                    ...
                    _button.FlatAppearance.BorderColor = (form.AcceptButton == _button) ? (pIsDarkMode ? Color.FromArgb(70, 200, 70) : Color.FromArgb(70, 155, 230)) : (pIsDarkMode ? Color.DarkGray : Color.Black);
            ...
            Label lblPrompt = new Label {
                Text = Message,
                Font = Overlay.GetMainFont(14),
                AutoSize = true,
                // BackColor = Icon != MsgIcon.Error ? Control.DefaultBackColor : (pIsDarkMode ? Color.DarkRed : Color.Red),
                // ForeColor = DMode.OScolors.TextActive,
        ...
        public static string GetCurrentLanguage(string pDefault = "en-US") {
            string _ret = pDefault;
            string CurrentLanguage = Utils.GetCultureInfo().ToString();
            if (IsCurrentLanguageSupported(new List<string> { "en-US", "fr-FR", "ko-KR", "ja-JP", "zh-CN", "zh-TW" }, CurrentLanguage)) {
        ...
        private static Dictionary<string, string> GetButtonTranslations(string pLanguage) {
            ...
            Dictionary<string, string> ButtonTranslations = new Dictionary<string, string> {
                { "en-US", "OK|Cancel|Yes|No|Continue|Retry|Abort|Ignore|Try" },
                { "fr-FR", "OK|Annuler|Oui|Non|Continuer|Réessayer|Abandonner|Ignorer|Essayer" },
                { "ko-KR", "OK|취소|예|아니오|계속|다시 시도|중단|무시|써 보다" },
                { "ja-JP", "OK|キャンセル|はい|いいえ|続行|再試行|中止|無視|試す" },
                { "zh-CN", "OK|取消|是|否|继续|重试|中止|忽略|尝试" },
                { "zh-TW", "OK|取消|是|否|繼續|重試|中止|忽略|嘗試" }
